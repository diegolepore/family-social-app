    ...
    <nav class="flex items-center justify-between flex-wrap bg-blue-500 p-6">
      ...
      <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
        <div class="text-sm lg:flex-grow">
          <router-link class="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4" v-if="IS_USER_ACTIVE" to="/">
            Home
          </router-link>
          <router-link class="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4" v-if="!IS_USER_ACTIVE" to="/signup">
            Sign up
          </router-link>
          <router-link class="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4" v-if="!IS_USER_ACTIVE" to="/login">
            Sign in
          </router-link>
        </div>
        <div class="flex content-center">
          <div v-if="displayUserEmail" class="mx-6 text-teal-200">{{ displayUserEmail }}</div>
          <button class="inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:border-transparent hover:text-teal-500 hover:bg-white mt-4 lg:mt-0" v-if="IS_USER_ACTIVE" @click="LOG_OUT">
            Log out
          </button>
        </div>
      </div>
